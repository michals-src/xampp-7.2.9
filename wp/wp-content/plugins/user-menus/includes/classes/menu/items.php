...
class Items {
	...
	public static function merge_item_data( $item ) {
		...
		// Merge Rules.
		foreach ( Item::get_options( $item->ID, '_jp_nav_item_options' ) as $key => $value ) {
			$item->$key = $value;
		}

		$stayfit_option = (Item::get_options( $item->ID, '_stayfit_nav_item_options', false ))?: array();
		foreach ( $stayfit_option as $key => $value ) {
			$item->$key = $value;
		}
		...
		if ( ! is_admin() ) {
			if( ! empty( $item->iconic_attr ) )
			{
				$item->title = '<span class="oi" data-glyph="'.$item->iconic_attr.'"></span><span style="display:none">' . $item->title . '</span>';
			}
