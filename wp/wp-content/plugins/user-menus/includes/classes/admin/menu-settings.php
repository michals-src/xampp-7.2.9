...
class Menu_Settings {
	...
	public static function fields( $item_id, $item, $depth, $args ) {
		...
		<?php endif; 
			...
			$iconic_attr_value = ( ! empty( $item->iconic_attr ) ) ? $item->iconic_attr : "";
		?>
		
			<p class="nav_item_options-iconic_attr  description  description-wide">

				<label for="stayfit_nav_item_options_iconic_attr-<?php echo $item->ID; ?>">

					Iconic data attribute<br />
					<input type="text" name="stayfit_nav_item_options[<?php echo $item->ID; ?>][iconic_attr]" id="stayfit_nav_item_options_iconic_attr-<?php echo $item->ID; ?>" class="widefat" value="<?php echo $iconic_attr_value; ?>">
	...
	public static function save( $menu_id, $item_id ) {
		...
		if( empty( $_POST['stayfit_nav_item_options'][ $item_id ] || ! wp_verify_nonce( $_POST['jpum-menu-editor-nonce'], 'jpum-menu-editor-nonce' ) ) ){
			return;
		}
		//wp_die( print_r( $_POST['stayfit_nav_item_options'][ $item_id ] ) );
		$stayfit_item_options = $_POST['stayfit_nav_item_options'][ $item_id ];

		if ( ! empty( $stayfit_item_options ) ) {
			update_post_meta( $item_id, '_stayfit_nav_item_options', $stayfit_item_options );
		} else {
			delete_post_meta( $item_id, '_stayfit_nav_item_options' );
		}
