...
add_action( 'admin_menu', 'cmm_add_submenu_page' );
...
function reorder_admin_jobs_callback()
{
	...
	<div id="job-sort" class="wrap">
		...
		<h2><?php echo _e('Sort Job Positions', 'wp-job-listing'); ?></h2><img src="<?php echo esc_url( admin_url() . '/images/loading.gif' ); ?>" id="loading-animation">
		<?php if($job_listing->have_posts()): ?>
			...
			<ul id="custom-type-list">
				<?php while($job_listing->have_posts()) : $job_listing->the_post(); ?>
					<li id="<?php esc_attr(the_id()); ?>"><?php if( get_the_title() ){ esc_html(the_title()); }else{ _e("<strong>( Bez tytułu )</strong>", "wp-job-listing"); } ?></li>
				<?php endwhile; ?>
			</ul>
		<?php else: ?>
...
add_action( 'wp_ajax_save_sort', 'cmm_save_reorder' );
