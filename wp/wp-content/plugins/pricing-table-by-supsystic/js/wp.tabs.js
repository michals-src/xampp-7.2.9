	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('ptsWpTabs')) {
					$this.addClass('ptsWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.ptsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.ptsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.ptsTabContent');
					allTabsContent.hide();
